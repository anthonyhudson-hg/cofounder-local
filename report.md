...
- **God component**: `EmployeeSettingsPanel.tsx` (65-459) does role/model/channels/behavior/prompt-preview/avatar-upload/department-creation in one 460-line function. `ResponsibilityRow` is already split out — the "Role"/"Model"/"Channels"/"Behavior" sections are natural candidates to follow that precedent. — ⚠️ NOT DONE: this file was already touched by four separate fixes in this pass (§1.2, §1.6, §4.1, §5.9); a structural decomposition on top of that is a real refactor with its own review surface, better done as its own dedicated change against a settled diff rather than layered into an already-large one. The one piece that *was* extracted — the manager-cycle BFS (`descendantIds`) — moved to `src/lib/orgChart.ts` specifically because it needed to be unit-tested (see Part 8).
- **Inline payload types drift from real shapes**: `domains/register.ts` mixes `cmd.payload.field` and a shorter `p<T>()` helper across ~40 handlers with no consistent convention — pure noise, and directly enabled the `roles: never[]` bug in §1.4. — ⚠️ NOT DONE: the two `roles: never[]` / `effort as never` instances that were *actual bugs* are fixed (§1.4); converting all ~40 handlers to one stylistic convention is a pure-refactor, zero-behavior-change mechanical pass with real diff size and no correctness payoff — lower priority than everything else in this pass.
- **`sidecar/tsconfig.json`**: no ESLint config exists for the sidecar at all (confirmed no `sidecar/eslint.config.*`, and root `eslint.config.js:10` explicitly excludes it) — the code that spawns subprocesses, holds API keys, and owns the SQLite DB has zero static-analysis coverage (no unused-var, no floating-promise, no unsafe-any checks). — ✅ DONE: extended the root `eslint.config.js` with a second block scoped to `sidecar/src/**/*.ts` (Node globals, no React plugins), reusing the already-installed eslint/typescript-eslint tooling instead of standing up a second toolchain. It immediately caught 5 real issues on first run: two empty `catch {}` blocks in a test's cleanup helper (`no-empty`) and three rethrows discarding the original error (`preserve-caught-error` — the same class of bug already fixed elsewhere in this pass) — all fixed.
- **`@types/node` version skew**: root `package.json` pins `^26.1.1`, `sidecar/package.json` pins `^22.0.0` — the sidecar is the one that actually runs on Node and should track the real target runtime; root only needs it for Vite tooling. — ✅ DONE: bumped `sidecar/package.json` to `^24.0.0`, matching the actual local Node runtime (`v24.14.1`); verified sidecar build/tests still pass. Root's own mismatch (types-only, Vite tooling) was left alone as the report itself judged it lower-priority.
- **New finding (not in original report)**: running `npm install` for the `@types/node` bump surfaced a **pre-existing high-severity `kysely` SQL-injection advisory** (GHSA-wmrf-hv6w-mr66 and two related CVEs, affecting `kysely<=0.28.16`, the version this project pins). The fix requires a breaking major bump to `kysely@0.29.3`, which touches every DB query in the sidecar — far too large and risky to force through unreviewed as part of this pass. **Not fixed here** — flagged for a dedicated upgrade effort with its own testing, not attempted under time pressure.
...
- **Zero component tests exist** for any of the ~27 files under `src/components/`. Highest-value gaps given what this review found: `OrgChartView.tsx`'s cycle handling and `EmployeeSettingsPanel.tsx`'s manager-cycle filter (§4.1 — exactly where a real bug was found), and `MessageList`'s memoization behavior (§5.2). — ✅ PARTIAL: the manager-cycle BFS logic itself (the actual bug-prone part) was extracted to `src/lib/orgChart.ts` and given 5 focused unit tests (including a cycle-termination test), avoiding the need for a full component-rendering test harness (no React Testing Library is set up in this project — adding one is its own infrastructure decision, not bundled in here). Full component/rendering tests for `OrgChartView`/`MessageList` remain **not done**.
- **`sidecar/src/__tests__/runtime.test.ts`**: substantive where it exists (cross-company leakage invariant, monotonic event sequencing, capability grant transitions, and a genuine end-to-end git-commit-through-approval-and-vault test are all real, meaningful assertions — noted positively) but has zero coverage of `ClaudeProvider`/`CodexProvider` (§3.7) and zero coverage of the `dispatch()`/wire-protocol boundary (§3.3). — ✅ PARTIAL: added a test exercising `dispatch()`/`validateInbound()` directly (proving the §1.4/§3.3 fix), now 10 sidecar tests. **Not done**: mocked-SDK tests for `ClaudeProvider`/`CodexProvider` — mocking `@anthropic-ai/claude-agent-sdk` and especially `@openai/codex-sdk` (loaded via the `new Function`-guarded dynamic `import()` documented in `codex.ts`) is a real test-infrastructure investment (module-boundary mocking across that ESM/CJS interop shim), not a quick addition; flagged rather than rushed.
- **`src/lib/*.test.ts`**: generally good, non-tautological (`emoji.test.ts` verifies every UI emoji has a bundled asset on disk; `sillyNames.test.ts` verifies actual shuffle unbiasedness and non-mutation). Gaps: `mentions.ts`'s asymmetric word-boundary check (checks trailing edge only, not leading) is untested; `reactionNotices.ts`'s `consumeReactionNotices` (the function that actually talks to the runtime) has no test at all, only its pure formatting half does. — ✅ DONE (both): the word-boundary asymmetry turned out to be a real bug, not just a test gap — fixed in `findMentions` to require a boundary on both edges (report §5.9), with new tests for both the fix and the pre-existing punctuation/start-of-string cases. `consumeReactionNotices` now has two tests (`vi.mock`'d `command`) covering the `fromName` fallback and the 120-char preview truncation.
...
## Post-cutover reassessment (2026-07-08)

Since the fix pass above, the app went through a full architectural cutover: the legacy client-orchestrated sidecar (`sidecar/src/index.ts`, raw JSON-over-stdio `cos_*` commands) was retired entirely, and DM send + channel send + token counting now run server-side in the runtime (`sendMessage`/`sendChannelMessage`/`countTokens`). That's a large enough change to the codebase's shape that it's worth re-checking every deferred item against it rather than assuming the old reasoning still holds. Verified each one concretely (re-read the current code, didn't just re-assert the old note) — findings below.

**Changed by the cutover:**

- **Part 8 — "no React Testing Library is set up" is no longer true.** `@testing-library/react`/`@testing-library/dom` (React 19-compatible) are now installed, and `useConversation.test.ts`/`useChannel.test.ts` are real integration tests (12 cases) exercising optimistic rendering, live delta application, post-send reconciliation, the never-stuck-`sending` guarantee, and both reaction.toggle regression cases — by mocking `runtimeClient`'s three functions, which the cutover made sufficient since these hooks are now thin transport layers rather than embedding prompt-composition logic. The infrastructure blocker this item cited is closed; full component-rendering tests for `OrgChartView`/`MessageList` are still not written (that's a separate, still-open piece of the same Part 8 finding).
- **A concrete UX regression was introduced and fixed within the same pass**: moving DM send to a single server-orchestrated command meant the client's optimistic assistant placeholder had `debug_payload: null` for the entire streaming duration (previously the client built and persisted the full debug payload synchronously before ever sending, so it was visible immediately). Fixed by having `sendMessage` emit an early `"meta"` delta (debug payload) right after the placeholder is persisted, before the provider call starts, with a dedicated sidecar test asserting it arrives before any `"text"` delta. Not a report finding originally (didn't exist yet), noted here since it's exactly the kind of gap this reassessment pass exists to catch.
- **§4.2 (process lifecycle) blast radius halved, gaps unchanged.** There is now exactly one Node child process instead of two, so "no auto-restart with backoff" and "no graceful shutdown handshake" apply to half as much surface as before. Neither gap is actually closed — a runtime crash still requires a full app relaunch, same as documented.
- **Part 6 — `register.ts`'s inline-payload-type-style inconsistency partially addressed, not fully.** The cutover's own new handlers (`command:message.send`'s extension, `command:message.sendChannel`) were written using the `CommandEnvelope`-cast style at first — i.e., the cutover was actively making this exact deferred finding worse before it got caught. Converted both to the more common `p<T>()` convention while the file was open for the cutover itself, closing that specific regression. The other ~15 pre-existing `CommandEnvelope`-cast handlers (`company.*`, `conversation.rename`, `tool.invoke`, `approval.resolve`, etc.) are untouched — the full mechanical normalization pass across all ~40 handlers remains correctly deferred as zero-behavior-change churn.
- **New finding from this reassessment, not previously flagged**: audited the actual `kysely` API surface used anywhere in `sidecar/src` against the three CVEs behind the still-open advisory (GHSA-wmrf-hv6w-mr66, GHSA-8cpq-38p9-67gx, GHSA-pv5w-4p9q-p3v2) — all three require `sql.lit()`, `JSONPathBuilder.key()`/`.at()`, or `Kysely<any>` to be reachable. Grepped the whole sidecar tree: **none of those three APIs are used anywhere in this codebase.** Real-world exposure today is effectively zero despite `npm audit` reporting a high-severity finding against the installed `kysely@0.27.6` — this downgrades urgency from "open SQL-injection risk in production" to "unused vulnerable surface in a transitive dependency, worth clearing on general principle." Still recommend the breaking `0.29.3` bump eventually (it's free defense-in-depth against a future contributor introducing one of these calls), but this is no longer a live security gap and shouldn't be treated as one.

**Reconfirmed unchanged (verified, not just re-asserted):**

- **§5.3 (ThreadPanel no live streaming)** — re-read `useThread.ts` and `ThreadPanel.tsx` in full. The architectural blocker is genuinely unrelated to the sidecar/runtime split: it's that `useThread` is its own poll-on-`sending`-flip hook, disconnected from `useConversation`'s live `messages` state and delta stream. Confirmed additionally that a threaded reply's assistant placeholder is never even added to `useConversation`'s local `messages` array in the first place (only top-level sends get an optimistic placeholder), so there's currently no live target for thread-scoped deltas to patch into even in principle — the fix still requires `useThread` to share state with `useConversation`/`useChannel` rather than poll independently. Correctly still deferred.
- **§5.4 (Composer draft persistence across conversation switches), §5.8 (modal auto-save vs. explicit-save unification), §4.6 (FK `ON DELETE` cascade), §4.9 (`repos.forCompany()` migration), §3.7/Part 8 (mocked-SDK provider tests), Part 6 (`EmployeeSettingsPanel` god-component split)** — none of these touch the chat send/transport path the cutover changed; re-read each site and confirmed the original reasoning for deferral still applies unchanged. Worth noting for §4.9 specifically: the cutover's new `sendMessage`/`sendChannelMessage` code follows the same existing direct-`ctx.db` pattern as the rest of `conversations/service.ts` rather than routing through `repos.forCompany()` — consistent with (not a new instance of) the already-deferred finding, not a regression.
- **§5.9 (typed `CommandMap`/`QueryMap`)** — the cutover added two more untyped command names (`message.sendChannel`, `tokenCount.count`) to the ~40 the protocol already had no compile-time payload/response mapping for, making the eventual fix marginally larger, not smaller. Still correctly deferred as a genuine type-system redesign, not a contained fix.

No further action taken on any "reconfirmed unchanged" item — they're listed to show they were actually re-examined, not silently carried forward.
